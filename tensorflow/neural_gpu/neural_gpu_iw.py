...
class NeuralGPU(object):
  ...
  def step_train(self,
    ...
    print("step", global_step, "forward samples..", flush=True)
    ...
    for sample in xrange(batch_size):
        ...
        grads_batch.append(res[1:offset])
    ...
    for i, grad_batch in enumerate(grads_batch):
            ...
            if i == 0:
                shape = (batch_size, grads_flatten.shape[0])
    ...
    alphas = self.solveForAlphas(sess,
                                 ...
                                 p, # p
                                 C, # C
                                 np.array(losses_batch)).astype(np.float32)
